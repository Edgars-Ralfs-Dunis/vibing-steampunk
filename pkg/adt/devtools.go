...
func (c *Client) ActivatePackage(ctx context.Context, packageName string, maxObjects int) (*ActivatePackageResult, error) {
	...
	for _, rec := range toActivate {
		...
		obj := rec.Object
		res, err := c.Activate(ctx, obj.URI, obj.Name)
		switch {
		case err != nil:
		...
		// A transport-level success says only that SAP answered. Reporting an
		// object as activated without checking res.Success produced counts like
		// "13 activated, 0 failed" for objects still sitting at VERSION 0.
		case res != nil && !res.Success:
			result.Failed = append(result.Failed, ActivationFailed{
				Name:   obj.Name,
				Type:   obj.Type,
				Reason: activationFailureReason(res),
			})
		default:
